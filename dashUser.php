<?php  
    ...
    if (!isset($_SESSION['user'])){
		...
		if(isset($_REQUEST)){
			$tuserFound = $db->prepare("SELECT guardado.id, guardado.id_especificacion, guardado.marca, guardado.modelo, marca.naMarca, modelo.namMdelo FROM guardado INNER JOIN marca ON guardado.marca = marca.id_marca INNER JOIN modelo ON guardado.modelo = modelo.id_modelo WHERE guardado.id_usuario = :id_user");
			$tuserFound->bindParam(":id_user", $_SESSION['user']['id_user']);
			$tuserFound->execute();
